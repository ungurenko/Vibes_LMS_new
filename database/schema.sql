...
CREATE TABLE dashboard_stages (
    ...
    description TEXT,  -- Описание для баннера на дашборде
    week_label VARCHAR(100),  -- "WEEK 01 // BASICS"
    is_active BOOLEAN NOT NULL DEFAULT FALSE,  -- Активный этап (показывается на дашборде)
    sort_order SMALLINT NOT NULL DEFAULT 0,
...
CREATE TABLE admin_calls (
    ...
    -- Напоминания
    reminders TEXT[] DEFAULT ARRAY[]::TEXT[],  -- ["24h", "1h", "15m"]
