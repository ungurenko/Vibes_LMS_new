      ...
      const fetchStages = async () => {
         try {
            ...
            if (result.success && result.data) {
               ...
               // Устанавливаем активную стадию (первую с статусом current или первую в списке)
               const currentStage = result.data.find((s: any) => s.status === 'current');
               setActiveStageId(currentStage?.id || result.data[0]?.id);

               // Получаем завершённые задачи пользователя
               ...
               <motion.div variants={cardVariants} className="md:col-span-1 bg-gradient-to-br from-emerald-500 to-teal-600 rounded-[2.5rem] p-6 text-white relative overflow-hidden group hover:scale-[1.02] transition-transform shadow-lg shadow-emerald-500/20">
                  ...
                  <div className="relative z-10 flex flex-col h-full">
                     ...
                     <div className="mt-6">
                        ...
                        <h3 className="text-lg font-bold leading-tight text-white/95">{upcomingCall.topic}</h3>
                     </div>
                  </div>
               </motion.div>
            ) : (
