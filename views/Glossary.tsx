...
const Glossary: React.FC<GlossaryProps> = ({ onNavigate, onAskAI }) => {
   ...
   const refreshRandomTerm = () => {
      ...
      do {
         const randomIndex = Math.floor(Math.random() * glossaryData.length);
         newTerm = glossaryData[randomIndex];
      } while (newTerm.id === randomTerm?.id && glossaryData.length > 1);
                  ...
                  <div className="relative z-10 flex flex-col md:flex-row gap-8 items-start justify-between">
                     ...
                     <div className="flex flex-col gap-3 shrink-0 w-full md:w-auto">
                        <Button
                           ...
                           className="px-6 py-4 h-auto rounded-2xl bg-white/10 dark:bg-black/5 hover:bg-white/20 dark:hover:bg-black/10 backdrop-blur-md border border-white/10 dark:border-black/10 text-white dark:text-black"
                        >
                        ...
                        <Button
                           ...
                           className="px-6 py-4 h-auto rounded-2xl border border-white/10 dark:border-black/10 hover:bg-white/5 dark:hover:bg-black/5 text-zinc-400 dark:text-zinc-500 text-sm"
                        >
                           <RefreshCw size={18} />
            ...
            <div className="flex overflow-x-auto scrollbar-none gap-2 justify-start md:justify-center py-2 px-4 md:px-0">
               {CATEGORIES.map((cat) => (
                  <button
                     ...
                     className={`px-5 py-2.5 rounded-xl text-sm font-bold transition-all whitespace-nowrap border ${activeCategory === cat
                           ...
                           <Card className="break-inside-avoid group relative bg-white dark:bg-zinc-900 rounded-3xl p-0 py-0 border border-zinc-200 dark:border-white/5 hover:border-purple-300 dark:hover:border-purple-500/30 transition-all hover:shadow-xl hover:shadow-purple-500/5 mb-6 gap-0 shadow-none">
                              <CardHeader className="p-6 pb-0">
                                 <div className="flex justify-between items-start">
                                    <div>
                                       <CardTitle className="font-display text-xl font-bold text-zinc-900 dark:text-white leading-tight">
                                          {item.term}
                                       </CardTitle>
                                       {item.slang && (
                                    ...
                                    <Badge variant="outline" className={`rounded-lg text-xs font-bold uppercase tracking-wider ${colorClass}`}>
                                       {item.category}
                                    </Badge>
                                 </div>
                              </CardHeader>
                                 ...
                                 <div className="flex items-center justify-between pt-4 border-t border-zinc-100 dark:border-white/5 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
                                    <div className="flex gap-2">
                                       ...
                                       <Button
                                          ...
                                          className="text-zinc-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-500/10 rounded-xl"
                                          title="Спросить ассистента"
                                       >
                                          <MessageSquare size={18} />
                                       </Button>
                                    </div>
                                    <span className="text-xs font-bold text-zinc-300 hover:text-zinc-900 dark:hover:text-white transition-colors cursor-default">
                                       #vibes
                                    </span>
                                 </div>
                              </CardContent>
