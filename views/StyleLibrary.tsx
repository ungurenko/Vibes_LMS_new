...
const StyleLibrary: React.FC = () => {
  ...
  return (
    <div className="max-w-[1600px] mx-auto px-4 md:px-8 py-8 md:py-12 pb-32">
        ...
        <div className="flex overflow-x-auto scrollbar-none gap-2 pb-1">
          {CATEGORIES.map((cat) => (
            <button
              ...
              className={`px-5 py-2.5 rounded-full text-sm font-medium transition-all duration-300 border whitespace-nowrap ${activeCategory === cat
                ...
                <div className="relative h-[65%] overflow-hidden bg-zinc-100 dark:bg-zinc-800">
                  ...
                  <div className="absolute inset-0 bg-black/40 backdrop-blur-[2px] hidden [@media(hover:hover)_and_(pointer:fine)]:flex opacity-0 group-hover:opacity-100 transition-opacity duration-300 items-center justify-center gap-3">
                    <Button
                      ...
                      className="rounded-full bg-white/10 border border-white/20 text-white backdrop-blur-md hover:bg-white hover:text-black hover:scale-110 shadow-lg"
                  ...
                  {/* Badge */}
                  <Badge className="absolute top-4 right-4 bg-black/50 backdrop-blur-md border-white/10 text-xs font-bold text-white uppercase tracking-wider shadow-lg">
                    {style.category}
                  </Badge>
                </div>
                  ...
                  <div className="flex items-end justify-between gap-3">
                    <div className="flex flex-wrap gap-2 flex-1">
                      {style.tags.slice(0, 3).map(tag => (
                        <Badge key={tag} variant="secondary" className="rounded-md bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 text-xs uppercase font-bold tracking-wider">
                          {tag}
                        </Badge>
                      ))}
                    </div>
      ...
      {/* Modal / Lightbox - rendered via Portal to escape stacking context */}
      {createPortal(
        <AnimatePresence>
          {selectedStyle && (
            <div className="fixed inset-0 z-[60] flex items-center justify-center px-4 py-4 md:py-8">
              ...
              >
                ...
                <Button
                  ...
                  size="icon"
                  onClick={() => setSelectedStyle(null)}
                  className="absolute top-4 right-4 z-20 rounded-full bg-black/20 text-white backdrop-blur-md hover:bg-black/40 min-w-[44px] min-h-[44px]"
                >
                  <X size={24} />
                </Button>
                ...
                <div className="w-full md:w-2/5 p-8 md:p-12 flex flex-col bg-white dark:bg-zinc-900 overflow-y-auto">
                  ...
                  <div className="mt-auto pt-6 border-t border-zinc-100 dark:border-white/5">
                    <Button
                      ...
                      className="w-full py-4 h-auto text-lg hover:opacity-90 active:scale-[0.98] shadow-lg"
                    >
