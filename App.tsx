...
const AppContent: React.FC = () => {
  ...
  // NEW: Global Modules State (Source of Truth for Lessons)
  const [modules, setModules] = useState<CourseModule[]>(() => {
      const saved = localStorage.getItem('vibes_course_modules');
      return saved ? JSON.parse(saved) : COURSE_MODULES;
  });
  ...
  // NEW: Persist Modules (Lessons/Video URLs)
  useEffect(() => {
    localStorage.setItem('vibes_course_modules', JSON.stringify(modules));
  }, [modules]);
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'admin-content': return <AdminContent modules={modules} onUpdateModules={setModules} />;
